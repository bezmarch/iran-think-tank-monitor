...
async def scan_source(session, source: dict, cutoff: datetime) -> tuple[list[dict], dict]:
    ...
    # Deduplicate within the source before returning.
    unique = {}
    for item in items:
        key = item.get("url", "").split("?", 1)[0].rstrip("/") or item.get("title", "").lower()
        unique[key] = item
...
async def main() -> None:
    ...
    previous_keys = {(i.get("url", "").rstrip("/") or i.get("source", "") + "|" + i.get("title", "").lower()) for i in existing_items}
    source_status=[]
    for group in groups:
