...
ARCHIVE_DAYS = 10
# Scan the full archive window so a replaced or missing data.json can be rebuilt.
SCAN_HOURS = ARCHIVE_DAYS * 24
MAX_SITEMAP_URLS = 50
MAX_ARTICLE_FETCHES_PER_SOURCE = 40
MAX_LISTING_LINKS_PER_SOURCE = 80
...
def article_like_url(url: str, source: dict) -> bool:
    """Return True for links likely to be individual articles rather than navigation pages."""
    parsed = urlparse(url)
    source_host = urlparse(source["url"]).netloc.lower().removeprefix("www.")
    host = parsed.netloc.lower().removeprefix("www.")
    if not host or not (host == source_host or host.endswith("." + source_host)):
        return False
    path = parsed.path.lower().rstrip("/")
    if not path or path.count("/") < 2:
        return False
    excluded = ("/video/", "/podcasts/", "/newsletters/", "/author/", "/authors/", "/search/", "/livecoverage/")
    if any(marker in path for marker in excluded):
        return False
    markers = source.get("article_url_markers", [])
    if markers and not any(marker.lower() in path for marker in markers):
        return False
    # Article slugs on major news sites normally contain several words or an ID/hash.
    tail = path.rsplit("/", 1)[-1]
    return tail.count("-") >= 3 or bool(re.search(r"[a-f0-9]{8,}$", tail))


async def discover_listing_urls(session: aiohttp.ClientSession, source: dict) -> list[str]:
    """Extract article links from explicitly configured section/latest pages.

    This is essential for large JavaScript-heavy/paywalled publishers whose RSS or
    sitemap discovery is incomplete. Only same-site, article-looking URLs are kept.
    """
    pages = source.get("listing_pages", [])
    if not pages:
        return []
    found: list[str] = []
    seen: set[str] = set()
    for page in pages:
        raw, _, final_url = await fetch(session, page)
        if not raw:
            continue
        soup = BeautifulSoup(raw, "html.parser")
        # Standard anchors plus canonical URLs embedded in JSON-LD/Next.js data.
        candidates = [urljoin(final_url, tag.get("href")) for tag in soup.find_all("a", href=True)]
        for script in soup.find_all("script"):
            text = script.string or script.get_text(" ", strip=True)
            if not text or len(text) > 5_000_000:
                continue
            candidates.extend(re.findall(r'https?://[^"\\\s<>]+', text))
            candidates.extend(urljoin(final_url, p) for p in re.findall(r'"(\/[^"\\]+)"', text))
        for candidate in candidates:
            candidate = candidate.split("#", 1)[0]
            if candidate in seen or not article_like_url(candidate, source):
                continue
            seen.add(candidate)
            found.append(candidate)
            if len(found) >= MAX_LISTING_LINKS_PER_SOURCE:
                return found
    return found
...
async def discover_sitemap_urls(session: aiohttp.ClientSession, source: dict, cutoff: datetime) -> list[tuple[str, datetime | None]]:
    ...
    for sitemap_url in candidates[:3]:
        ...
        # Follow recent/relevant child sitemaps rather than merely the first five.
        if root.tag.endswith("sitemapindex"):
            child_records = []
            for sitemap_node in root.iter():
                if not sitemap_node.tag.endswith("sitemap"):
                    continue
                child = next((n.text.strip() for n in sitemap_node if n.tag.endswith("loc") and n.text), None)
                modified = next((parse_date(n.text) for n in sitemap_node if n.tag.endswith("lastmod") and n.text), None)
                if child:
                    child_records.append((child, modified))
            include_markers = [m.lower() for m in source.get("sitemap_include", [])]
            if include_markers:
                preferred = [r for r in child_records if any(m in r[0].lower() for m in include_markers)]
                if preferred:
                    child_records = preferred
            child_records.sort(key=lambda r: r[1] or datetime.min.replace(tzinfo=timezone.utc), reverse=True)
            for child, child_modified in child_records[:20]:
                if child_modified and child_modified < cutoff - timedelta(days=1):
                    continue
...
async def scan_article_page(session: aiohttp.ClientSession, source: dict, url: str, fallback_date: datetime | None, cutoff: datetime) -> dict | None:
    ...
    # Many modern publishers expose reliable metadata only through JSON-LD.
    for script in soup.find_all("script", attrs={"type":"application/ld+json"}):
        ...
        for node in expanded:
            if not isinstance(node, dict):
                continue
            date_values.extend(str(node.get(k)) for k in ("datePublished", "dateCreated", "dateModified") if node.get(k))
            if not title and node.get("headline"):
                title = clean(str(node.get("headline")))
            if not summary and node.get("description"):
                summary = clean(str(node.get("description")))
    published = next((dt for dt in (parse_date(v) for v in date_values) if dt), fallback_date)
    if not title or not published or published < cutoff or published > now() + timedelta(hours=3):
        return None
    body_text = clean(" ".join(p.get_text(" ", strip=True) for p in soup.find_all("p")[:30]))
...
async def scan_source(session, source: dict, cutoff: datetime) -> tuple[list[dict], dict]:
    ...
    # Explicit section/latest pages supplement feeds, rather than being used only after failure.
    listing_urls = await discover_listing_urls(session, source)
    if listing_urls:
        page_results = await asyncio.gather(*(
            scan_article_page(session, source, url, None, cutoff)
            for url in listing_urls[:MAX_ARTICLE_FETCHES_PER_SOURCE]
        ), return_exceptions=True)
        listing_items = [item for item in page_results if isinstance(item, dict)]
        items.extend(listing_items)
        if listing_items:
            methods.append("listing")

    # Fall back to sitemaps when feeds/listings found nothing, or when a source asks
    # for supplemental sitemap coverage (useful for high-value publishers).
    sitemap_url_count = 0
    if not items or source.get("supplement_sitemap", False):
        ...
        sitemap_items = [item for item in page_results if isinstance(item, dict)]
        items.extend(sitemap_items)
        if sitemap_items:
            methods.append("sitemap")

    # Deduplicate feed, listing-page and sitemap discoveries before returning.
    ...
    return items, {
        ...
        "listing_links": len(listing_urls),
        "sitemap_links": sitemap_url_count,
    }


TRACKING_QUERY_KEYS = {
    "st", "mod", "reflink", "utm_source", "utm_medium", "utm_campaign",
    "utm_term", "utm_content", "utm_id", "campaign", "link", "share",
    "output", "rss", "tesla", "gaa_at", "gaa_n", "gaa_ts", "gaa_sig"
}

def canonical_url(url: str) -> str:
    """Return a stable article URL for deduplication.

    Removes fragments and tracking parameters while preserving query parameters that
    may genuinely identify content. WSJ's stable article id in the path is retained.
    """
    ...
    try:
        ...
        for k, v in parse_qsl(parts.query, keep_blank_values=True):
            kl = k.lower()
            if kl in TRACKING_QUERY_KEYS or kl.startswith("utm_") or kl.startswith("gaa_"):
                continue
            kept.append((k, v))
...
def normalized_title(title: str) -> str:
    ...
    title = re.sub(r"\b(video|live|watch|photos?|podcast|explainer)\s*[:|—-]\s*", "", title)
    title = re.sub(r"[^a-z0-9]+", " ", title)
    return " ".join(title.split())

def article_path_id(url: str) -> str:
    """Extract stable publisher article IDs where available."""
    path = canonical_url(url).split("?", 1)[0]
    # WSJ article ids and similarly structured publisher ids, e.g. a59f9699.
    m = re.search(r"-([a-f0-9]{8,16})(?:$|/)", path, re.I)
    return m.group(1).lower() if m else ""

def choose_better_item(old: dict, new: dict) -> dict:
    """Keep the richer record; prefer a canonical/clean URL and fuller summary."""
    def score(item: dict) -> tuple:
        url = item.get("url", "")
        summary = item.get("summary", "") or ""
        return (
            bool(item.get("published_utc")),
            len(summary),
            -len(url.split("?", 1)[1]) if "?" in url else 1,
            item.get("published_utc", ""),
        )
    winner = new if score(new) >= score(old) else old
    winner = dict(winner)
    winner["url"] = canonical_url(winner.get("url", "")) or winner.get("url", "")
    return winner

def deduplicate_items(items: list[dict]) -> list[dict]:
    """Remove exact URL variants and same-source headline duplicates.

    This deliberately does not collapse matching reports from different publishers;
    those belong in story clusters.
    """
    exact: dict[str, dict] = {}
    for item in items:
        item = dict(item)
        item["url"] = canonical_url(item.get("url", "")) or item.get("url", "")
        source = (item.get("source") or "").strip().lower()
        aid = article_path_id(item.get("url", ""))
        title = normalized_title(item.get("title", ""))
        key = f"{source}|id:{aid}" if aid else (f"{source}|url:{item['url']}" if item.get("url") else f"{source}|title:{title}")
        exact[key] = choose_better_item(exact[key], item) if key in exact else item

    # Second pass catches multiple publisher URLs or feed/listing variants carrying
    # the same or very slightly altered title from the same source and publication day.
    result: list[dict] = []
    buckets: dict[tuple[str, str], list[int]] = {}
    for item in sorted(exact.values(), key=lambda x: x.get("published_utc", ""), reverse=True):
        source = (item.get("source") or "").strip().lower()
        dt = parse_date(item.get("published_utc"))
        day = dt.date().isoformat() if dt else "unknown"
        title = normalized_title(item.get("title", ""))
        merged = False
        for idx in buckets.get((source, day), []):
            other = result[idx]
            otitle = normalized_title(other.get("title", ""))
            if not title or not otitle:
                continue
            seq = SequenceMatcher(None, title, otitle).ratio()
            ta, tb = set(title.split()), set(otitle.split())
            jac = len(ta & tb) / max(1, len(ta | tb))
            if title == otitle or seq >= 0.94 or jac >= 0.88:
                result[idx] = choose_better_item(other, item)
                merged = True
                break
        if not merged:
            buckets.setdefault((source, day), []).append(len(result))
            result.append(item)
    return result
...
async def main() -> None:
    ...
    # Scan every enabled source on every hourly run. Earlier priority scheduling made
    # the dashboard appear to lose sources and reduced rediscovery after data.json was replaced.
    ...
    for item in existing_items:
        ...
        if published and published >= archive_cutoff:
            ...
            item["content_group"], item["content_kind"] = classify_content(
                classification_source, item.get("title", ""), item.get("url", ""), item.get("summary", "")
            )
            preserved_items.append(item)

    previous_fingerprints = {
        ((i.get("source") or "").lower(), article_path_id(i.get("url", "")) or canonical_url(i.get("url", "")) or normalized_title(i.get("title", "")))
        for i in deduplicate_items(preserved_items)
    }
    source_status=[]
    discovered_items: list[dict] = []
    for group in groups:
        if isinstance(group, tuple):
            group_items, status = group
            source_status.append(status)
            discovered_items.extend(group_items)

    # Clean duplicates already present in the archive and merge all newly discovered records.
    items = deduplicate_items(preserved_items + discovered_items)
    current_fingerprints = {
        ((i.get("source") or "").lower(), article_path_id(i.get("url", "")) or canonical_url(i.get("url", "")) or normalized_title(i.get("title", "")))
        for i in items
    }
    # Store the archive newest-first. The website can still re-sort it by importance, source or oldest-first.
    items = sorted(items, key=lambda i: i.get("published_utc", ""), reverse=True)
    type_counts={}
    country_counts={}
    language_counts={}
    content_counts={}
    ...
    new_item_count = len(current_fingerprints - previous_fingerprints)
    ...
    print(
        f"Preserved {len(existing_items)} existing items; "
        f"saved {len(items)} items after checking {len(due_sources)} of {len(SOURCES)} sources to {OUTPUT}"
    )
